...
export default function LobbyPage() {
  ...
  const hasAutoCalledRef = React.useRef(false);
  ...
  React.useEffect(() => {
    ...
    if (invitedBy && !isLoading && !hasAutoCalledRef.current) {
      const inviter = allOnlineUsers.find(
        (u) => u.username.toLowerCase() === invitedBy.toLowerCase()
      );
      if (inviter) {
        hasAutoCalledRef.current = true;
        initiateCall(inviter, 'video');
        setLocation('/lobby', { replace: true });
      }
    }
  }, [currentUser, invitedBy, isLoading, allOnlineUsers, initiateCall, setLocation]);
